"""Sermon media helpers — external URL parsing and MP4 duration checks.

Independent from gallery_media.py. Sermons and Gallery remain separate systems.
"""
...
def parse_sermon_external_url(url: str) -> SermonExternalVideoInfo:
    ...
    if yt_id and re.fullmatch(r"[\w-]{6,}", yt_id):
        ...
        return SermonExternalVideoInfo(
            provider="youtube",
            video_id=yt_id,
            watch_url=watch,
...
def get_sermon_mp4_duration_seconds(file_storage) -> float | None:
    """Read MP4 duration from the mvhd atom. Returns seconds or None."""
...
def format_sermon_duration(seconds: float | None) -> str:
    if seconds is None:
        return ""
